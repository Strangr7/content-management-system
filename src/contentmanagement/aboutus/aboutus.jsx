...
const Aboutus = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        // Sort data by createdAt (or reverse the array if no date is available)
  ...
  const handleUpdate = (id) => {
    navigate(`/contentmanagement/aboutus/updateabout/${id}`);
  };

  const handleDelete = (id) => {
    if (window.confirm("Are you sure you want to delete this item")) {
      axios
        .delete(`http://localhost:5000/api/aboutpage/${id}`)
        .then(() => {
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {aboutData.map((item) => (
                  <tr key={item._id}>
                    ...
                    <td className="px-6 py-4 whitespace-wrap text-sm text-gray-900">{item.description}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {item.image && (
                        <img src={`http://localhost:5000${item.image}`} alt="about" className="h-12 w-12 rounded-md" />
                      )}
                    </td>
